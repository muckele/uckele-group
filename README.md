...
## Daily Deal Hunter Digest

The Daily Deal Hunter Digest is one internal, server-owned Morning Briefing prepared every calendar day—including Saturday and Sunday—at 08:00 in the IANA timezone `America/Los_Angeles`. “Pacific business date” means that timezone's calendar date; the MVP does not skip holidays. Every scheduler tick and the existing protected trigger use one identity, `daily-deal-hunter-email:<Pacific YYYY-MM-DD>`, and one existing scheduler registration. A date can complete with exactly one mutually exclusive result: `normal-digest` or `required-source-alert`.

The required Google Sheet is primary authority. Deal OS is optional supplemental authority. A healthy required source produces the current Acquisition Inbox counts and at most five items from the existing `needs-review` / `acquisition-priority` ordering; the copy is deterministic, bounded, and never AI-authored. Optional Deal OS degradation still permits a primary-backed briefing with a bounded warning. Required-source failure produces **ACTION REQUIRED**, no trusted counts or recommendations, and labels persisted rows as last known. The digest does not decide Pursue/Watch/Pass, mutate CRM, request or send a CIM, contact a broker, run a follow-up, or execute Stage 2.

Administrators and viewers may read the bounded Morning Briefing and sanitized Operations authority. Viewers remain read-only. Only an authenticated administrator can reach the existing zero-payload admin trigger, which is not a normal UI workflow; the server also rechecks required-source authority on protected triage mutations. No client can select a recipient, sender, subject, body, HTML, tags, provider, notification type, or idempotency key.

The canonical `scheduled_job_runs` state machine is `pending` (pre-provider work; recoverable only after one hour), `transmitting` (provider boundary crossed; reconciliation only), `failed` (definitive nonacceptance; exact-envelope retry no earlier than `nextRetryAt`, 30 minutes after failure), `ambiguous` (acceptance unresolved; never retransmit), and `completed` (terminal). The exact prepared envelope and deterministic idempotency key are persisted before the token-fenced transmitting transition. Production requires Resend and signed, replay-safe webhook evidence; EmailJS is not a Daily Digest fallback. Reconciliation accepts only exact evidence from the durable marker, local email event, signed Resend event, or bounded read-only provider lookup. Zero proof remains unresolved before the one-hour window and becomes ambiguous afterward; one unique identity may complete; multiple identities require high attention and no resend. Late exact evidence may reconcile `ambiguous` to `completed` without another provider call.

Operations exposes status, attempts, fresh versus stale pending, retry/reconciliation timing, marker agreement, required-source status, and high-attention ambiguity—never the prepared envelope, address, claim token, raw provider payload, or a manual resend control. The database row is primary authority; the address-free marker is recovery evidence at `DEAL_HUNTER_DAILY_EMAIL_MARKER_DIR` (normally `/data/deal-hunter-daily-email`). See [backend setup](docs/backend-setup.md) for configuration and state details and [deployment](docs/deployment.md) for the separately authorized production checklist, evidence-preserving rollback, and first-natural-08:00 smoke procedure. Do not send an extra manual smoke digest after deployment.
