...
import { DEAL_HUNTER_CRM_INTEGRITY_F02_CONFIRMATION } from '../server/repairs/dealHunterCrmIntegrityF02.js';
import {
...
const commandName = 'deal-hunter-crm-integrity-f02-repair';
...
export function parseDealHunterCrmIntegrityF02Args(args = []) {
  const acceptedFlags = [
    '--apply',
    '--actor',
    '--reason',
  ...
  if (parsed.confirmation !== DEAL_HUNTER_CRM_INTEGRITY_F02_CONFIRMATION) {
    throw new Error(`Apply requires --confirm ${DEAL_HUNTER_CRM_INTEGRITY_F02_CONFIRMATION}.`);
  }
  return parsed;
}
...
export async function runDealHunterCrmIntegrityF02Cli({
  ...
  const options = parseDealHunterCrmIntegrityF02Args(argv);
...
async function main() {
  const result = await runDealHunterCrmIntegrityF02Cli();
