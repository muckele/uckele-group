...
import { DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_CONFIRMATION } from '../server/repairs/dealHunterCrmIntegrityF02Repair3.js';
import {
...
const commandName = 'deal-hunter-crm-integrity-f02-repair3';
...
export function parseDealHunterCrmIntegrityF02Repair3Args(args = []) {
  const acceptedFlags = [
    '--apply',
    '--actor',
    '--reason',
  ...
  if (parsed.confirmation !== DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_CONFIRMATION) {
    throw new Error(`Apply requires --confirm ${DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_CONFIRMATION}.`);
  }
  return parsed;
}
...
export async function runDealHunterCrmIntegrityF02Repair3Cli({
  ...
  const options = parseDealHunterCrmIntegrityF02Repair3Args(argv);
...
async function main() {
  const result = await runDealHunterCrmIntegrityF02Repair3Cli();
