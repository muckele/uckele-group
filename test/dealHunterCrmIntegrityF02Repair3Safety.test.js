...
import {
  ...
  createDealHunterCrmIntegrityF02Repair3TestAuthority,
  fingerprintDealHunterCrmIntegrityF02Repair3RawRow,
} from '../server/repairs/dealHunterCrmIntegrityF02Repair3.js';
import {
  applyDealHunterCrmIntegrityF02Repair3,
  evaluateDealHunterCrmIntegrityF02Repair3DeclaredOwner,
  inspectDealHunterCrmIntegrityF02Repair3,
} from '../server/services/dealHunterCrmIntegrityF02Repair3.js';
...
const execution = {
  ...
  reason: 'Repair the three reviewed F-02 Repair3 CRM integrity defects.',
...
function contextFingerprint(row) {
  ...
  return rawFingerprint(projected);
}
...
function syntheticAuthority(database) {
  const relationships = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships;
  ...
  return createDealHunterCrmIntegrityF02Repair3TestAuthority({
    ...
    collisionOpportunity: contextFingerprint(rawRow(database, 'deal_hunter_opportunities', 'opportunity_id', collision.opportunityId)),
    reviewedUnrelatedOpportunity: contextFingerprint(rawRow(
      database,
...
function expectedAfterSnapshot(before) {
  ...
  const mutations = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.mutations;
...
async function createIncidentFixture(t) {
  ...
  const collision = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships.collision;
  const backlink = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships.backlink;
  const managedName = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships.managedName;
  const reviewedUnrelatedOpportunity =
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships.reviewedUnrelatedOpportunity;
  try {
    ...
    insertSubmission(database, {
      ...
      company: DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.mutations[2].before,
      opportunityId: null,
      metadata: {
        dealHunter: {
          managed: true,
    ...
    // Model the proven ordinary full-backfill timestamp refresh after authority review.
    database.prepare('UPDATE deal_hunter_opportunities SET updated_at = ? WHERE opportunity_id IN (?, ?)')
      .run('2026-09-14T15:00:49.165Z', collision.opportunityId, reviewedUnrelatedOpportunity.opportunityId);
...
async function approvedDryRun(fixture, overrides = {}) {
  return inspectDealHunterCrmIntegrityF02Repair3({
...
async function approvedApply(fixture, dryRun, backup, overrides = {}) {
  return applyDealHunterCrmIntegrityF02Repair3({
    databasePath: fixture.databasePath,
    provider: 'sqlite',
    authority: fixture.authority,
    reviewedManifest: dryRun.approvalManifest,
    expectedPlanChecksum: dryRun.planChecksum,
    confirmation: DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_CONFIRMATION,
...
test('Repair2 refuses the same fixture timestamp drift that Repair3 accepts without writes', async (t) => {
  const { createDealHunterCrmIntegrityF02Repair2TestAuthority } = await import('../server/repairs/dealHunterCrmIntegrityF02Repair2.js');
  const { inspectDealHunterCrmIntegrityF02Repair2 } = await import('../server/services/dealHunterCrmIntegrityF02Repair2.js');
  const fixture = await createIncidentFixture(t);
  const before = snapshotDatabase(fixture.databasePath);
  const raw = { ...fixture.authority.rowFingerprints };
  for (const [label, id] of [
    ['collisionOpportunity', fixture.authority.relationships.collision.opportunityId],
    ['reviewedUnrelatedOpportunity', fixture.authority.relationships.reviewedUnrelatedOpportunity.opportunityId],
  ]) {
    const later = before.deal_hunter_opportunities.find((row) => row.opportunity_id === id);
    const historical = { ...later, updated_at: '2026-09-12T10:00:00.000Z' };
    assert.deepEqual(Object.keys(later).filter((key) => later[key] !== historical[key]), ['updated_at']);
    raw[label] = rawFingerprint(historical);
  }
  const result = await inspectDealHunterCrmIntegrityF02Repair2({
    databasePath: fixture.databasePath, provider: 'sqlite', ...execution,
    authority: createDealHunterCrmIntegrityF02Repair2TestAuthority(raw),
  });
  assert.equal(result.status, 'refused');
  assert.deepEqual(result.blockers.sort(), [
    'AUTHORITY_FINGERPRINT_MISMATCH:collisionOpportunity',
    'AUTHORITY_FINGERPRINT_MISMATCH:reviewedUnrelatedOpportunity',
  ]);
  assert.equal((await approvedDryRun(fixture)).status, 'repair-required');
  assert.deepEqual(snapshotDatabase(fixture.databasePath), before);
});

test('Repair3 service refuses every non-timestamp context field, metadata identity, and Brake relationship drift', async (t) => {
  const relationships = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships;
  for (const [label, opportunityId] of [
    ['collisionOpportunity', relationships.collision.opportunityId],
    ['reviewedUnrelatedOpportunity', relationships.reviewedUnrelatedOpportunity.opportunityId],
  ]) {
    const fields = ['created_at', 'opportunity_id', 'canonical_name', 'canonical_recipient', 'canonical_location', 'primary_submission_id', 'identity_version', 'status'];
    const metadataFields = ['listingIds', 'sourceIds', 'lastObservedDealKey', 'lastObservedListingUrl'];
    for (const field of [...fields, ...metadataFields]) {
      await t.test(`${label}.${field}`, async (subtest) => {
        ...
        if (fields.includes(field)) {
          database.prepare(`UPDATE deal_hunter_opportunities SET ${field} = ? WHERE opportunity_id = ?`)
            .run('unreviewed-change', opportunityId);
        } else {
          const metadata = JSON.parse(rawRow(database, 'deal_hunter_opportunities', 'opportunity_id', opportunityId).metadata);
          if (field === 'listingIds' || field === 'sourceIds') {
            metadata.identitySnapshot = { ...metadata.identitySnapshot, [field]: ['unreviewed-change'] };
          } else metadata[field] = 'unreviewed-change';
          database.prepare('UPDATE deal_hunter_opportunities SET metadata = ? WHERE opportunity_id = ?')
            .run(JSON.stringify(metadata), opportunityId);
        }
        database.close();
        const before = snapshotDatabase(fixture.databasePath);
        const result = await approvedDryRun(fixture);
        assert.equal(result.status, 'refused');
        assert.ok(result.blockers.includes(`${field === 'opportunity_id' ? 'AUTHORITY_ROW_MISSING' : 'CONTEXT_PROJECTION_FINGERPRINT_MISMATCH'}:${label}`), result.blockers.join(', '));
        assert.deepEqual(snapshotDatabase(fixture.databasePath), before);
      });
    }
  }
  await t.test('Brake gains CRM import ownership', async (subtest) => {
    const fixture = await createIncidentFixture(subtest);
    const database = new Database(fixture.databasePath, { fileMustExist: true });
    database.prepare('UPDATE deal_hunter_crm_imports SET opportunity_id = ? WHERE id = ?')
      .run(relationships.reviewedUnrelatedOpportunity.opportunityId, 'fixture-unclaimed-import');
    database.close();
    const result = await approvedDryRun(fixture);
    assert.equal(result.status, 'refused');
    assert.ok(result.blockers.includes('REVIEWED_UNRELATED_OPPORTUNITY_IMPORT_COUNT_MISMATCH'));
  });
});

test('Repair3 keeps updated_at strict on every remaining raw authority row', async (t) => {
  const { collision, backlink, managedName } = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships;
  for (const [label, table, key, id] of [
    ['legacyCollisionImport', 'deal_hunter_crm_imports', 'id', collision.legacyImportId],
    ['retainedUrlImport', 'deal_hunter_crm_imports', 'id', collision.retainedImportId],
    ['collisionSubmission', 'contact_submissions', 'id', collision.sharedSubmissionId],
    ['backlinkOpportunity', 'deal_hunter_opportunities', 'opportunity_id', backlink.opportunityId],
    ['backlinkImport', 'deal_hunter_crm_imports', 'id', 'fixture-backlink-authority-import'],
    ['backlinkSubmission', 'contact_submissions', 'id', backlink.submissionId],
    ['nameMismatchSubmission', 'contact_submissions', 'id', managedName.submissionId],
  ]) {
    await t.test(label, async (subtest) => {
      const fixture = await createIncidentFixture(subtest);
      const database = new Database(fixture.databasePath, { fileMustExist: true });
      database.prepare(`UPDATE ${table} SET updated_at = ? WHERE ${key} = ?`).run('2026-09-14T15:00:49.165Z', id);
      database.close();
      const result = await approvedDryRun(fixture);
      assert.equal(result.status, 'refused');
      assert.ok(result.blockers.includes(`AUTHORITY_FINGERPRINT_MISMATCH:${label}`));
    });
  }
});

test('F-02 Repair3 raw row fingerprints preserve SELECT-star property order exactly', () => {
  const rawRow = {
    id: 'row-1',
    opportunity_id: null,
  ...
  assert.equal(
    fingerprintDealHunterCrmIntegrityF02Repair3RawRow(rawRow),
...
test('F-02 Repair3 authority fixes the three mutations and all diagnostic guards', () => {
  assert.equal(DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.evidence.release, '123');
  assert.equal(
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.evidence.revision,
    '39d23f1d50c138acf24f8603a2f258852e3f116e',
  );
  assert.deepEqual(
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.evidence.supersession.opportunityCount,
    {
      before: 475,
      after: 476,
  ...
  assert.deepEqual(
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.evidence.supersession
  ...
  assert.deepEqual(DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.mutations, [
    {
  ...
  assert.deepEqual(DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.expectedAuditBefore, {
    imports: 32,
    opportunities: 476,
  ...
  assert.equal(Object.keys(DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.rowFingerprints).length, 7);
  assert.equal(
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.rowFingerprints.legacyCollisionImport,
    '0650ae104b7123bd94f76772bcd4895c6a9ac088443ac3b37b3f33ee4897ae84',
  );
  assert.equal(
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.contextOpportunityFingerprints.collisionOpportunity,
    'e1f8e123556493101a6c9407d3df40c9034a181917c60cc0b1dd27d68581add3',
  );
  assert.equal(
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.rowFingerprints.collisionSubmission,
    '6b811304f09d44b3737f73b2a479654c4a5c4b45fee4b5e2b6e67542eb674082',
  );
  assert.equal(
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.contextOpportunityFingerprints.reviewedUnrelatedOpportunity,
    '7be900911fb0192da9d89fd1bc089eca774a8a735b4fd3a6c37661807248b3e0',
  );
  assert.deepEqual(
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships.collision.claimantImportIds,
    [
  ...
  assert.equal(
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships.collision.declaredOwnerSemantics,
    'direct-or-metadata-with-no-conflict',
  );
  assert.equal(
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships.reviewedUnrelatedOpportunity.listingId,
    'costar:2552696',
  );
});

test('F-02 Repair3 declared-owner predicate accepts only the six reviewed shapes', () => {
  const expectedOpportunityId =
    DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships.collision.opportunityId;
...
test('F-02 Repair3 service enforces every collision declared-owner shape', async (t) => {
  const collision = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships.collision;
  const cases = [
...
test('F-02 Repair3 plan checksum binds approvals but excludes incidental runtime values', () => {
  const approval = {
    actor: 'Owner Reviewer',
    reason: 'Repair the three reviewed F-02 Repair3 CRM integrity defects.',
  ...
  const first = createDealHunterCrmIntegrityF02Repair3ApprovalManifest({
    ...approval,
    generatedAt: '2026-09-13T01:00:00.000Z',
    databasePath: '/tmp/one.sqlite',
  });
  const second = createDealHunterCrmIntegrityF02Repair3ApprovalManifest({
    ...approval,
  ...
  assert.notEqual(
    first.planChecksum,
    createDealHunterCrmIntegrityF02Repair3ApprovalManifest({ ...approval, actor: 'Another Reviewer' }).planChecksum,
  );
  assert.equal(first.plan.requiredConfirmation, DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_CONFIRMATION);
  assert.equal(Object.hasOwn(first.plan, 'generatedAt'), false);
  assert.equal(Object.hasOwn(first.plan, 'databasePath'), false);
});

test('F-02 Repair3 dry run reads the exact unsafe incident without durable writes or PII output', async (t) => {
  const fixture = await createIncidentFixture(t);
  const before = snapshotDatabase(fixture.databasePath);
...
test('F-02 Repair3 apply changes exactly three cells and produces the real clean audit', async (t) => {
  const fixture = await createIncidentFixture(t);
  ...
  const after = snapshotDatabase(fixture.databasePath);
  // collision submission mutation: NONE; neither context opportunity changes.
  const collisionId = fixture.authority.relationships.collision.sharedSubmissionId;
  assert.deepEqual(after.contact_submissions.find((row) => row.id === collisionId), before.contact_submissions.find((row) => row.id === collisionId));
  assert.deepEqual(after.deal_hunter_opportunities, before.deal_hunter_opportunities);
  ...
  for (const [label, id] of [
    ...
    const raw = rawFingerprint(before.deal_hunter_opportunities.find((row) => row.opportunity_id === id));
    assert.equal(result.receipt.beforeFingerprints[label], raw);
    assert.equal(result.receipt.afterFingerprints[label], raw);
  }
...
test('F-02 Repair3 refuses partial state, target drift, authority drift, and wrong global counts', async (t) => {
  const cases = [
    {
      name: 'partial state',
      mutate(database) {
        const mutation = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.mutations[0];
        database.prepare('UPDATE deal_hunter_crm_imports SET opportunity_id = ? WHERE id = ?')
    ...
    },
    {
      ...
      mutate(database) {
        database.prepare('UPDATE contact_submissions SET updated_at = ? WHERE id = ?')
          .run('2026-09-13T09:00:00.000Z', DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.mutations[1].id);
      },
    },
    {
      ...
      mutate(database) {
        database.prepare('UPDATE deal_hunter_crm_imports SET source_name = ? WHERE id = ?')
          .run('drifted-source', DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships.collision.retainedImportId);
      },
    },
    {
...
test('F-02 Repair3 refuses every reviewed authority and global-guard drift', async (t) => {
  const relationships = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.relationships;
  const scenarios = [
    {
    ...
    {
      name: 'collision opportunity status drift',
      blocker: /CONTEXT_PROJECTION_FINGERPRINT_MISMATCH:collisionOpportunity/,
      mutate(database) {
        database.prepare('UPDATE deal_hunter_opportunities SET status = ? WHERE opportunity_id = ?')
          .run('inactive', relationships.collision.opportunityId);
      },
    },
    {
...
test('F-02 Repair3 refuses unsupported target triggers and non-SQLite providers', async (t) => {
  const fixture = await createIncidentFixture(t);
  const database = new Database(fixture.databasePath, { fileMustExist: true });
  database.exec(`
...
test('F-02 Repair3 apply requires exact approval, execution identity, and current matching backup evidence', async (t) => {
  const fixture = await createIncidentFixture(t);
  const dryRun = await approvedDryRun(fixture);
  const backup = await createFixtureBackup(fixture);
  const cases = [
...
test('F-02 Repair3 backup snapshot and live database must both reproduce the reviewed preflight', async (t) => {
...
test('Repair3 context timestamps remain strict in reviewed backup and live logical digests', async (t) => {
  const fixture = await createIncidentFixture(t);
  const reviewed = await approvedDryRun(fixture);
  const backup = await createFixtureBackup(fixture);
  const database = new Database(fixture.databasePath, { fileMustExist: true });
  database.prepare('UPDATE deal_hunter_opportunities SET updated_at = ? WHERE opportunity_id = ?')
    .run('2026-09-15T15:00:49.165Z', fixture.authority.relationships.collision.opportunityId);
  database.close();
  const changed = snapshotDatabase(fixture.databasePath);
  const fresh = await approvedDryRun(fixture);
  assert.equal(fresh.status, 'repair-required');
  assert.notEqual(fresh.planChecksum, reviewed.planChecksum);
  await assert.rejects(approvedApply(fixture, reviewed, backup), /live database.*preflight digest/i);
  await assert.rejects(approvedApply(fixture, fresh, backup), /backup snapshot.*preflight/i);
  assert.deepEqual(snapshotDatabase(fixture.databasePath), changed);
});

test('F-02 Repair3 rolls back completely after every mutation boundary and on CAS conflict', async (t) => {
...
test('F-02 Repair3 rolls back when the post-repair audit is not clean', async (t) => {
  const fixture = await createIncidentFixture(t);
  const dryRun = await approvedDryRun(fixture);
  const backup = await createFixtureBackup(fixture);
  const before = snapshotDatabase(fixture.databasePath);
...
test('F-02 Repair3 rolls back when the final foreign-key check fails', async (t) => {
  const fixture = await createIncidentFixture(t);
  const dryRun = await approvedDryRun(fixture);
  const backup = await createFixtureBackup(fixture);
  const before = snapshotDatabase(fixture.databasePath);
...
test('F-02 Repair3 exposes no writable live-database callback before commit', async (t) => {
  const fixture = await createIncidentFixture(t);
  const dryRun = await approvedDryRun(fixture);
  const backup = await createFixtureBackup(fixture);
  const before = snapshotDatabase(fixture.databasePath);
...
test('F-02 Repair3 normal startup creates only the deferred unique index and refreshes cached ownership health', async (t) => {
  const fixture = await createIncidentFixture(t);
  const dryRun = await approvedDryRun(fixture);
  const backup = await createFixtureBackup(fixture);
  ...
  const legacyClaim = schemaAfter.prepare('SELECT opportunity_id FROM deal_hunter_crm_imports WHERE id = ?')
    .get(DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_AUTHORITY.mutations[0].id);
  schemaAfter.close();
...
test('F-02 Repair3 CLI is dry-run by default and exposes no arbitrary mutation selectors', async () => {
  const { parseDealHunterCrmIntegrityF02Repair3Args } = await import(
    '../scripts/repair-deal-hunter-crm-integrity-f02-repair3.js'
  );
  const required = [
  ...
  const dryRun = parseDealHunterCrmIntegrityF02Repair3Args(required);

  assert.equal(dryRun.apply, false);
  assert.equal(dryRun.actor, execution.actor);
  assert.equal(dryRun.reason, execution.reason);
  assert.throws(
    () => parseDealHunterCrmIntegrityF02Repair3Args([...required, '--submission-id', 'operator-selected']),
    /unknown option|submission-id/i,
  );
  for (const flag of ['--ignored-fields', '--context-projection', '--authority', '--fingerprint', '--opportunity-id']) {
    assert.throws(() => parseDealHunterCrmIntegrityF02Repair3Args([...required, flag, 'operator-selected']), /unknown option/i);
  }
  assert.throws(
    () => parseDealHunterCrmIntegrityF02Repair3Args([...required, '--apply']),
    /checksum|manifest|backup|confirm/i,
  );
});

test('F-02 Repair3 stale and foreign-key-invalid backups are refused', async (t) => {
...
test('F-02 Repair3 rejects tampered reviewed manifests and wrong-provider apply', async (t) => {
  const fixture = await createIncidentFixture(t);
  const dryRun = await approvedDryRun(fixture);
  const backup = await createFixtureBackup(fixture);
...
test('F-02 Repair3 repeated apply on the exact corrected state performs zero writes', async (t) => {
  const fixture = await createIncidentFixture(t);
  const dryRun = await approvedDryRun(fixture);
  const backup = await createFixtureBackup(fixture);
  await approvedApply(fixture, dryRun, backup);
...
test('F-02 Repair3 CLI consumes its retained dry-run artifact without exposing synthetic authority flags', async (t) => {
  const fixture = await createIncidentFixture(t);
  const { runDealHunterCrmIntegrityF02Repair3Cli } = await import(
    '../scripts/repair-deal-hunter-crm-integrity-f02-repair3.js'
  );
  const commonArgs = [
    '--actor', execution.actor,
  ...
  const dryRun = await runDealHunterCrmIntegrityF02Repair3Cli({
    argv: commonArgs,
    getConfigFn,
    authority: fixture.authority,
    generatedAt: '2026-09-13T01:00:00.000Z',
  ...
  const applied = await runDealHunterCrmIntegrityF02Repair3Cli({
    argv: [
      ...commonArgs,
      '--apply',
      ...
      '--backup', backup.path,
      '--confirm', DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR3_CONFIRMATION,
    ],
    getConfigFn,
    authority: fixture.authority,
    completedAt: '2026-09-13T03:00:00.000Z',
  ...
  const verified = await runDealHunterCrmIntegrityF02Repair3Cli({
...
test('F-02 Repair3 already-correct apply still refuses unrelated live drift from the reviewed digest', async (t) => {
  const fixture = await createIncidentFixture(t);
  const dryRun = await approvedDryRun(fixture);
  const backup = await createFixtureBackup(fixture);
  await approvedApply(fixture, dryRun, backup);
...
test('F-02 Repair3 apply binds the inspected backup bytes to the verified manifest checksum', async (t) => {
  const fixture = await createIncidentFixture(t);
  const dryRun = await approvedDryRun(fixture);
  const backup = await createFixtureBackup(fixture);
  const before = snapshotDatabase(fixture.databasePath);
