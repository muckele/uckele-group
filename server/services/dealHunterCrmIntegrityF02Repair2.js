...
import {
  DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_APPROVAL_SCHEMA,
  DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_AUTHORITY,
  DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_CONFIRMATION,
  DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_RECEIPT_SCHEMA,
...
} from '../repairs/dealHunterCrmIntegrityF02Repair2.js';
...
function authorityFingerprintBlockers(rows, authority, valueState) {
  ...
  for (const [label, expected] of Object.entries(authority.rowFingerprints)) {
    ...
    if (fingerprintDealHunterCrmIntegrityF02Repair2RawRow(fingerprintInput) !== expected) {
...
// Repair2-only incident predicate. The generic audit remains unchanged: this
...
export function evaluateDealHunterCrmIntegrityF02Repair2DeclaredOwner({
  directOpportunityId,
...
function relationshipBlockers(database, rows, authority, valueState, selected) {
  ...
  const declaredOwner = evaluateDealHunterCrmIntegrityF02Repair2DeclaredOwner({
...
function maskedLogicalDatabaseDigest(database, authority) {
  ...
  for (const table of tables) {
    const rows = database.prepare(`SELECT * FROM ${table}`).all().map((row) => {
      ...
      return { ...row, [mutation.field]: '__F02_REPAIR2_AUTHORIZED_CELL__' };
    });
    rows.sort((left, right) => JSON.stringify(left).localeCompare(JSON.stringify(right)));
    contents.push({ table, rows });
  }
  return deterministicDealHunterCrmIntegrityF02Repair2Checksum(contents);
}

function actualFingerprints(rows) {
  return Object.fromEntries(Object.entries(rows).map(([label, row]) => [
    label,
    row ? fingerprintDealHunterCrmIntegrityF02Repair2RawRow(row) : '',
  ]));
}

function receiptMatches(receipt, manifest, postStateFingerprints) {
  if (!receipt || receipt.schema !== DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_RECEIPT_SCHEMA) return false;
  const { receiptChecksum, ...receiptBody } = receipt;
  if (!/^[a-f0-9]{64}$/.test(String(receiptChecksum || ''))) return false;
  if (deterministicDealHunterCrmIntegrityF02Repair2Checksum(receiptBody) !== receiptChecksum) return false;
  return receipt.incident === manifest.plan.incident
    ...
    && deterministicDealHunterCrmIntegrityF02Repair2Checksum(receipt.afterFingerprints)
      === deterministicDealHunterCrmIntegrityF02Repair2Checksum(postStateFingerprints);
}
...
async function analyzeOpenDatabase(database, {
  ...
  const approvalManifest = createDealHunterCrmIntegrityF02Repair2ApprovalManifest({
    actor,
    reason,
    executionRelease,
    toolingRevision,
...
export async function inspectDealHunterCrmIntegrityF02Repair2({
  databasePath,
  provider = 'sqlite',
  authority = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_AUTHORITY,
  actor,
  reason,
  executionRelease,
  toolingRevision,
...
function assertReviewedManifest({
  ...
  if (!reviewedManifest || reviewedManifest.schema !== DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_APPROVAL_SCHEMA) {
    throw new Error('Apply refused: a reviewed F-02 Repair2 approval manifest is required.');
  }
  ...
  if (reviewedManifest.planChecksum !== expectedPlanChecksum
    || deterministicDealHunterCrmIntegrityF02Repair2Checksum(reviewedManifest.plan) !== expectedPlanChecksum) {
    throw new Error('Apply refused: reviewed manifest checksum mismatch.');
  }
  const expected = createDealHunterCrmIntegrityF02Repair2ApprovalManifest({
    actor,
    reason,
    executionRelease,
    toolingRevision,
...
function createReceipt({ manifest, backup, beforeFingerprints, afterFingerprints, audit, completedAt }) {
  const body = {
    schema: DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_RECEIPT_SCHEMA,
  ...
  return {
    ...body,
    receiptChecksum: deterministicDealHunterCrmIntegrityF02Repair2Checksum(body),
  };
}

export async function applyDealHunterCrmIntegrityF02Repair2({
  databasePath,
  provider = 'sqlite',
  authority = DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_AUTHORITY,
  actor,
  ...
  if (confirmation !== DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_CONFIRMATION) {
    throw new Error(`Apply refused: confirmation must be ${DEAL_HUNTER_CRM_INTEGRITY_F02_REPAIR2_CONFIRMATION}.`);
  }
    ...
    for (let index = 0; index < authority.mutations.length; index += 1) {
      ...
      if (testHooks?.forceCasConflictAt === mutationNumber) {
        ...
        database.prepare(`UPDATE ${mutation.table} SET ${mutation.field} = ? WHERE ${identityColumn} = ?`)
          .run('__F02_REPAIR2_TEST_CAS_CONFLICT__', mutation.id);
      }
      const identityColumn = mutation.table === 'deal_hunter_opportunities' ? 'opportunity_id' : 'id';
