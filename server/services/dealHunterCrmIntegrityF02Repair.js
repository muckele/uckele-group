...
import {
  DEAL_HUNTER_CRM_INTEGRITY_F02_APPROVAL_SCHEMA,
  DEAL_HUNTER_CRM_INTEGRITY_F02_AUTHORITY,
  DEAL_HUNTER_CRM_INTEGRITY_F02_CONFIRMATION,
  DEAL_HUNTER_CRM_INTEGRITY_F02_RECEIPT_SCHEMA,
...
} from '../repairs/dealHunterCrmIntegrityF02.js';
...
function selectAuthorityRows(database, authority) {
  const { collision, backlink, managedName } = authority.relationships;
...
function authorityFingerprintBlockers(rows, authority, valueState) {
  ...
  for (const [label, expected] of Object.entries(authority.rowFingerprints)) {
    ...
    if (fingerprintDealHunterCrmIntegrityF02RawRow(fingerprintInput) !== expected) {
...
function relationshipBlockers(database, rows, authority, valueState) {
  const blockers = [];
  const { collision, backlink, managedName } = authority.relationships;
  ...
  const expectedCollisionIds = [collision.legacyImportId, collision.retainedImportId].sort();
  if (collisionImports.length !== (valueState === 'before' ? 2 : 1)) {
    blockers.push('COLLISION_IMPORT_COUNT_MISMATCH');
  }
  if (valueState === 'before'
    && JSON.stringify(collisionImports.map((row) => row.id).sort()) !== JSON.stringify(expectedCollisionIds)) {
  ...
  if (rows.collisionOpportunity?.primary_submission_id !== collision.sharedSubmissionId
    || rows.collisionSubmission?.deal_hunter_opportunity_id !== collision.opportunityId) {
    blockers.push('COLLISION_PRIMARY_RELATIONSHIP_MISMATCH');
  }
  const collisionMetadata = parseJson(rows.collisionSubmission?.metadata, {});
  if (collisionMetadata?.dealHunter?.opportunityId !== collision.opportunityId) {
    blockers.push('COLLISION_METADATA_AUTHORITY_MISMATCH');
  }
...
function maskedLogicalDatabaseDigest(database, authority) {
  ...
  for (const table of tables) {
    const rows = database.prepare(`SELECT * FROM ${table}`).all().map((row) => {
      ...
      return { ...row, [mutation.field]: '__F02_AUTHORIZED_CELL__' };
    });
    rows.sort((left, right) => JSON.stringify(left).localeCompare(JSON.stringify(right)));
    contents.push({ table, rows });
  }
  return deterministicDealHunterCrmIntegrityF02Checksum(contents);
}

function actualFingerprints(rows) {
  return Object.fromEntries(Object.entries(rows).map(([label, row]) => [
    label,
    row ? fingerprintDealHunterCrmIntegrityF02RawRow(row) : '',
  ]));
}

function receiptMatches(receipt, manifest, postStateFingerprints) {
  if (!receipt || receipt.schema !== DEAL_HUNTER_CRM_INTEGRITY_F02_RECEIPT_SCHEMA) return false;
  const { receiptChecksum, ...receiptBody } = receipt;
  if (!/^[a-f0-9]{64}$/.test(String(receiptChecksum || ''))) return false;
  if (deterministicDealHunterCrmIntegrityF02Checksum(receiptBody) !== receiptChecksum) return false;
  return receipt.incident === manifest.plan.incident
    ...
    && deterministicDealHunterCrmIntegrityF02Checksum(receipt.afterFingerprints)
      === deterministicDealHunterCrmIntegrityF02Checksum(postStateFingerprints);
}
...
async function analyzeOpenDatabase(database, {
  ...
  if (valueState !== 'mixed') {
    blockers.push(...relationshipBlockers(database, selected.rows, authority, valueState));
  }
  ...
  const approvalManifest = createDealHunterCrmIntegrityF02ApprovalManifest({
    actor,
    reason,
    executionRelease,
    toolingRevision,
  ...
  return {
    ...
    maskedLogicalDigest,
    rows: selected.rows,
    preStateFingerprints: authority.rowFingerprints,
...
export async function inspectDealHunterCrmIntegrityF02Repair({
  databasePath,
  provider = 'sqlite',
  authority = DEAL_HUNTER_CRM_INTEGRITY_F02_AUTHORITY,
  actor,
  reason,
  executionRelease,
  toolingRevision,
...
function assertReviewedManifest({
  ...
  if (!reviewedManifest || reviewedManifest.schema !== DEAL_HUNTER_CRM_INTEGRITY_F02_APPROVAL_SCHEMA) {
    throw new Error('Apply refused: a reviewed F-02 approval manifest is required.');
  }
  ...
  if (reviewedManifest.planChecksum !== expectedPlanChecksum
    || deterministicDealHunterCrmIntegrityF02Checksum(reviewedManifest.plan) !== expectedPlanChecksum) {
    throw new Error('Apply refused: reviewed manifest checksum mismatch.');
  }
  const expected = createDealHunterCrmIntegrityF02ApprovalManifest({
    actor,
    reason,
    executionRelease,
    toolingRevision,
...
function createReceipt({ manifest, backup, beforeFingerprints, afterFingerprints, audit, completedAt }) {
  const body = {
    schema: DEAL_HUNTER_CRM_INTEGRITY_F02_RECEIPT_SCHEMA,
  ...
  return {
    ...body,
    receiptChecksum: deterministicDealHunterCrmIntegrityF02Checksum(body),
  };
}

export async function applyDealHunterCrmIntegrityF02Repair({
  databasePath,
  provider = 'sqlite',
  authority = DEAL_HUNTER_CRM_INTEGRITY_F02_AUTHORITY,
  actor,
  ...
  if (confirmation !== DEAL_HUNTER_CRM_INTEGRITY_F02_CONFIRMATION) {
    throw new Error(`Apply refused: confirmation must be ${DEAL_HUNTER_CRM_INTEGRITY_F02_CONFIRMATION}.`);
  }
  ...
  try {
    ...
    for (let index = 0; index < authority.mutations.length; index += 1) {
      ...
      if (testHooks?.forceCasConflictAt === mutationNumber) {
        ...
        database.prepare(`UPDATE ${mutation.table} SET ${mutation.field} = ? WHERE ${identityColumn} = ?`)
          .run('__F02_TEST_CAS_CONFLICT__', mutation.id);
      }
      const identityColumn = mutation.table === 'deal_hunter_opportunities' ? 'opportunity_id' : 'id';
    ...
    const executionReceipt = createReceipt({
      manifest,
      backup,
      beforeFingerprints: authority.rowFingerprints,
