      ...
      <Reveal className="panel p-5 sm:p-6" data-admin-tour="deal-hunter-source-state">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2" data-admin-tour="deal-hunter-cim-workflow">
            ...
            {!readOnly ? <>
              <button className={secondaryButton} disabled={busy} onClick={onRunFullBackfill} type="button"><RefreshCw className="h-4 w-4" />Score Full Backfill</button>
              <button className={secondaryButton} disabled={busy || !sourceReviewComplete || crmEligible === 0} onClick={onSyncHighFits} type="button"><Upload className="h-4 w-4" />{crmSyncing ? 'Syncing…' : `Sync ${crmEligible} High Fit${crmEligible === 1 ? '' : 's'} to CRM`}</button>
              <button className={primaryButton} disabled={busy || !outboundReady || !sourceReviewComplete || !review?.totals?.cimReady} onClick={onOpenApprovals} type="button"><ClipboardList className="h-4 w-4" />Review CIM Requests</button>
              ...
              <button className={secondaryButton} disabled={busy || !outboundReady || !sourceReviewComplete} onClick={onSendEmail} type="button"><Send className="h-4 w-4" />{sending ? 'Sending…' : 'Send Internal Daily Summary'}</button>
            </> : null}
          </div>
        </div>
