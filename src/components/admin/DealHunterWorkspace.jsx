...
function sourceIsRequired(source = {}) {
  return source.required === true || source.sourceRole === 'required-primary' || String(source.id || '').startsWith('sheet-');
}

function sourceIsUnavailable(source = {}) {
  return source.mode !== 'retired' && (source.fetched === false || Boolean(source.error) || (sourceIsRequired(source) && Number(source.rowCount || 0) <= 0));
}
...
function DealSourceStatus({ source }) {
  ...
  const retired = source.mode === 'retired';
  const unavailable = sourceIsUnavailable(source);
  const required = sourceIsRequired(source);
  const sourceTone = retired ? 'info' : source.disabled ? 'warning' : !unavailable ? 'success' : setupRequired || !required ? 'warning' : 'danger';
  const sourceStatus = retired ? 'retired' : source.disabled ? 'disabled' : !unavailable ? `${source.rowCount || 0} rows` : setupRequired ? 'setup needed' : !required ? 'not used' : 'failed';
  const sourceRole = retired ? 'retired' : required ? 'required' : source.sourceRole === 'optional-supplemental' || source.required === false ? 'supplemental' : '';
  ...
  return (
    <div className="rounded-2xl border border-line bg-white/75 p-4 text-sm">
      <div className="flex flex-wrap gap-2">
        ...
        {sourceRole ? <Pill>{sourceRole}</Pill> : null}
      </div>
      ...
      {source.error ? <p className={`mt-2 ${setupRequired || !required ? 'text-amber-800' : 'text-red-700'}`}>{source.error}</p> : null}
...
function DealOsImportPanel({ importing, onImport, policy = {} }) {
  ...
  const [runFullBackfill, setRunFullBackfill] = useState(false);
  ...
  return (
    <form className="mt-5 rounded-2xl border border-sky-200 bg-sky-50/65 p-5" onSubmit={submit}>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <div className="flex items-start gap-3 rounded-xl border border-sky-200 bg-white/70 p-4 text-sm text-ink"><input aria-describedby="deal-os-full-backfill-help" aria-label="Score all current listings after import" checked={runFullBackfill} className="mt-1 h-4 w-4" disabled={importing} onChange={(event) => setRunFullBackfill(event.target.checked)} type="checkbox" /><span><strong className="block">Score all current listings after import</strong><span className="mt-1 block text-xs font-normal leading-5 text-ink/60" id="deal-os-full-backfill-help">Runs a full, scoring-only backfill once. It does not create CRM records or send email.</span></span></div>
      </div>
...
function ImportReviewSummary({ summary }) {
...
function CrmReconciliationPanel({ summary, reconciliation, busy, readOnly, available, unavailableReason, onPreview, onExecute }) {
  ...
  return (
    <section aria-label="Deal OS CRM reconciliation" className="rounded-2xl border border-moss/20 bg-moss/5 p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button className={secondaryButton} disabled={busy || !available} onClick={onPreview} type="button"><RefreshCw className={reconciliation?.busy ? 'h-4 w-4 animate-spin' : 'h-4 w-4'} />{preview ? 'Refresh preview' : 'Build preview'}</button>
          <button className={primaryButton} disabled={busy || !available || !preview || blocked || Number(counts.mutable || 0) === 0} onClick={onExecute} type="button"><Upload className="h-4 w-4" />Execute exact plan</button>
        </div>
      </div>
      {!available ? <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900" role="status">{unavailableReason || 'Deal OS reconciliation is unavailable until a fresh import is selected.'}</p> : null}
      {preview ? <>
    ...
    <section className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4 sm:p-5" id="cim-approvals">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          {actionsDisabled ? <p className="mt-2 text-sm font-semibold text-amber-900" role="status">Complete a successful required Google Sheet review before approving or sending CIM requests.</p> : null}
        </div>
      ...
      {!readOnly ? (
        <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-ink/68">{actionsDisabled ? 'Outreach stays paused while a required Google Sheet is unavailable.' : `This final action sends ${approvedDeals.length} first-contact email${approvedDeals.length === 1 ? '' : 's'} and starts their follow-up sequences.`}</p>
...
export default function DealHunterWorkspace({
  ...
  const followUpsSafe = (emailReadiness ? emailReadiness.followUpsSafe : true)
    && !review?.cimOutreachPause?.paused;
  const unavailableSources = (review?.sources || []).filter(sourceIsUnavailable);
  const unavailableRequiredSources = unavailableSources.filter(sourceIsRequired);
  const unavailableOptionalSources = unavailableSources.filter((source) => !unavailableRequiredSources.includes(source));
  const requiredSourceReviewComplete = unavailableRequiredSources.length === 0
    && (review?.sources || []).some(sourceIsRequired);
  const dealOsSource = (review?.sources || []).find((source) => source.id === 'deal-os-export');
  const dealOsReconciliationAvailable = requiredSourceReviewComplete
    && Boolean(dealOsSource)
    && !sourceIsUnavailable(dealOsSource);
  const reconciliationUnavailableReason = !requiredSourceReviewComplete
    ? 'Deal OS reconciliation is unavailable while a required Google Sheet is unavailable.'
    : 'Deal OS reconciliation is unavailable until a fresh import is selected.';
  const crmEligible = Number(review?.crmSyncPreview?.count ?? review?.totals?.crmEligible ?? 0);
  return (
    <section className="section-shell mt-5">
      <Reveal className="panel p-5 sm:p-6" data-admin-tour="deal-hunter-source-state">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div data-admin-tour="deal-hunter-review-buckets"><SectionLabel>Deal Hunter Scoring</SectionLabel><div className="mt-2 flex flex-wrap items-center gap-2"><h2 className="text-2xl font-semibold text-ink">Daily source review</h2>{review ? <Pill>{review.reviewMode === 'full-backfill' ? 'Full backfill' : `${review.lookbackDays || 0}-day daily`}</Pill> : null}</div><p className="mt-2 max-w-3xl text-sm leading-7 text-ink/68">Scores supported source fields against the acquisition profile. Full backfill, CRM sync, internal email, and CIM outreach remain separate actions.</p></div>
          <div className="flex flex-wrap gap-2" data-admin-tour="deal-hunter-cim-workflow">
            <button className={secondaryButton} disabled={busy} onClick={() => onReview?.()} type="button"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />{loading ? 'Reviewing…' : 'Review Recent'}</button>
            {!readOnly ? <>
              <button className={secondaryButton} disabled={busy || !requiredSourceReviewComplete} onClick={onRunFullBackfill} type="button"><RefreshCw className="h-4 w-4" />Score Full Backfill</button>
              <button className={secondaryButton} disabled={busy || !requiredSourceReviewComplete || crmEligible === 0} onClick={onSyncHighFits} type="button"><Upload className="h-4 w-4" />{crmSyncing ? 'Syncing…' : `Sync ${crmEligible} High Fit${crmEligible === 1 ? '' : 's'} to CRM`}</button>
              <button className={primaryButton} disabled={busy || !outboundReady || !requiredSourceReviewComplete || !review?.totals?.cimReady} onClick={onOpenApprovals} type="button"><ClipboardList className="h-4 w-4" />Review CIM Requests</button>
              <button className={secondaryButton} disabled={busy || !followUpsSafe} onClick={onRunFollowUps} type="button"><MailCheck className="h-4 w-4" />{followUpRunning ? 'Checking…' : followUpsSafe ? 'Run Follow-Ups' : 'Follow-Ups Paused'}</button>
              <button className={secondaryButton} disabled={busy || !outboundReady || !requiredSourceReviewComplete} onClick={onSendEmail} type="button"><Send className="h-4 w-4" />{sending ? 'Sending…' : 'Send Internal Daily Summary'}</button>
            </> : null}
          </div>
        </div>
