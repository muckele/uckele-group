...
### Daily Digest authority and operations

Every calendar day, including weekends and holidays, the existing single in-process scheduler becomes due at 08:00 `America/Los_Angeles`. It derives the Pacific calendar date with the IANA timezone rather than a fixed UTC offset and claims `daily-deal-hunter-email:<Pacific YYYY-MM-DD>`. Startup after 08:00 targets that same date. One date may complete as either `normal-digest` or `required-source-alert`, never both.

The required Google Sheet is primary authority. A healthy required source produces a normal Morning Briefing using the current Acquisition Inbox summary and at most five `needs-review` rows in existing `acquisition-priority` order. Missing, stale, or unavailable Deal OS remains an optional degradation: its data is excluded and a bounded warning is shown, but the Sheet-backed briefing remains usable. Failed or untrustworthy required authority produces `ACTION REQUIRED` with no trusted counts, opportunity names, scores, financial/contact data, or recommendations. Persisted rows are explicitly last known. After fixing the source, use **Deal Hunter → Operations** (`?view=operations`) for source review; a completed same-date alert remains final, so normal operation resumes on the next Pacific date.

The Morning Briefing appears in the existing Acquisition Inbox; there is no new page or navigation item. Operations shows only sanitized authority: `pending`, `transmitting`, `failed`, `ambiguous`, or `completed`; attempt and bounded timing information; fresh versus one-hour-stale pending; durable `nextRetryAt`; reconciliation state; marker agreement; required/optional source status; and high-attention conflicts. It never returns recipient/envelope content, claim tokens, raw provider payloads, raw source diagnostics, or a manual resend control.

`scheduled_job_runs` is the primary durable authority. `pending` means no provider boundary has been crossed and only a one-hour-stale row may be reclaimed with a new fenced token. `transmitting` means the boundary was durably crossed and the row is reconciliation-only. `failed` is reserved for definitive nonacceptance and becomes retryable no earlier than 30 minutes after the actual failure; retry reuses the exact persisted envelope and idempotency key. `ambiguous` is unresolved acceptance and never authorizes another send. `completed` is terminal. The exact immutable envelope is stored before the transmitting transition, and each accepted execution attempt calls the provider at most once.

Reconciliation checks exact, bounded evidence: the same-date durable marker, a matching local `email_events` record, a signed/replay-safe Resend event, then a bounded read-only provider lookup. Zero exact proof before the one-hour window remains unresolved; zero after it becomes ambiguous. One unique provider identity may complete. Multiple identities are a high-attention conflict and must not resend. Late exact evidence may move ambiguous to completed without another provider invocation. The address-free marker normally lives at `/data/deal-hunter-daily-email/<Pacific YYYY-MM-DD>.json`; a missing marker never overrides a completed database row.

The recipient is resolved only by the server from `DEAL_HUNTER_EMAIL_RECIPIENT`, falling back to `ADMIN_EMAIL`, and is verified operationally only as configured/valid plus a one-way fingerprint or suffix. Production-enabled Daily Digest readiness requires `DELIVERY_PROVIDER=resend`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `RESEND_WEBHOOK_SECRET` or `EMAIL_WEBHOOK_SECRET`, `PUBLIC_SITE_URL`, a valid effective recipient, exact `DEAL_HUNTER_DAILY_EMAIL_TIME=08:00`, `DEAL_HUNTER_DAILY_EMAIL_TIMEZONE=America/Los_Angeles`, positive `DEAL_HUNTER_DAILY_EMAIL_CHECK_INTERVAL_MS`, a retry interval of at least 1,800,000 ms, and a durable/writable `DEAL_HUNTER_DAILY_EMAIL_MARKER_DIR`. `DEAL_HUNTER_CRON_SECRET` is optional and only protects the existing external zero-payload trigger. EmailJS is not a Daily Digest fallback; readiness fails closed when required delivery or reconciliation configuration is missing or invalid. Resend evidence uses bounded tags `source=daily-deal-hunter`, `business_date=YYYY-MM-DD`, and `notification=normal-digest|required-source-alert`.

Both administrator and viewer can read the bounded Morning Briefing and Operations projection. A viewer cannot trigger or mutate. Only an authenticated administrator can reach the existing admin trigger, and neither it nor the optional secret cron route accepts recipient, sender, subject, body, HTML, tags, provider, notification type, or idempotency key. React state is not authorization: protected triage mutations recheck required-source authority on the server. Digest preparation may update machine-owned source/scoring/currentness evidence but cannot write operator decisions, CRM activity, CIM state, follow-up state, or Stage 2 state, and it performs no broker/outreach send.
...
The production Fly machine has one in-app Daily Digest scheduler registration. Admin, in-process scheduler, and optional external cron triggers share the canonical date claim; process-local guards are only an optimization.

Apply all committed Supabase migrations before deploying this version when `STORAGE_PROVIDER=supabase` is used. In particular, `20260904120000_daily_digest_scheduled_job_fencing.sql` adds service-role-only `claim_scheduled_job` and `transition_scheduled_job` functions over the existing table; it adds no table or column. Do **not** apply that PostgreSQL/Supabase migration to SQLite production. A future Supabase deployment must apply it through the normal migration process and verify execute permissions remain revoked from `public`, `anon`, and `authenticated` and granted only to `service_role`. Do not add schema beyond the reviewed function migration. SQLite uses the existing table and transactional/token-fenced adapter behavior; take and verify an application-consistent backup before any authorized production rollout.
...
The Daily Digest path does not import listings into CRM or mark legacy `deal_hunter_seen_deals` history. CRM synchronization and CIM/materials controls remain separate, explicit administrator workflows with their existing authority checks. The digest only projects the refreshed, canonical Acquisition Inbox and records its own scheduled-job, marker, and email evidence.
