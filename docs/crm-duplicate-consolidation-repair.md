...
## V3 row-authority correction

This is the current operator contract for the fixed Pooler/Berlin repair. The V1 and V2 sections above remain historical provenance, not alternate current artifacts or confirmations. V3 changes only the granularity of row-content authority: the complete schema inventory still covers **every** application table, while row digests, row counts, the authoritative row bound, and row-level incident-reference scanning cover every application table **except exactly** `analytics_events` and `contact_rate_limit_events`. Postconditions compare unchanged authoritative tables by row digest; the three mutation tables retain specialized exact checks. Both excluded tables must exist, their schemas remain authoritative, and a new table is authoritative by default. Their rows are neither counted nor selected for V3 plan authority; an incident-like token in either excluded table does not enter the reference inventory. The same token in an authoritative unclassified TEXT surface remains a blocker. An excluded-table schema change or any authoritative-table row change still refuses.

The closed, checksummed `rowAuthority` policy is `crm-duplicate-consolidation-row-authority-v1` with policy `schema-bound-row-content-excluded` and the ordered `excludedRowTables` value `["analytics_events", "contact_rate_limit_events"]`. There is no operator, CLI, environment, or inferred override. The V3 plan names its row-content facts `database.authorityLogicalDigest`, `database.authorityTotalRows`, and `authorityTableDigests`; the complete all-table `schema.digest` and SQLite integrity checks remain in force. V2's `database.logicalDigest`, `database.totalRows`, and `tableDigests` are not V3 aliases.

Current versioned values are:

- repair `UG-P7-01D-CRM-DUPLICATE-CONSOLIDATION-V3`;
- plan `crm-duplicate-consolidation-plan-v3`;
- manifest `crm-duplicate-consolidation-manifest-v3`;
- manifest namespace `crm-duplicate-consolidation:v3:`;
- confirmation `APPLY-UG-P7-01D-CRM-DUPLICATE-CONSOLIDATION-V3`;
- unchanged approval `crm-duplicate-consolidation-approval-v1` and checkpoint `crm-duplicate-consolidation-checkpoint-v1`.

V1/V2 artifacts, manifests, plans, and confirmations cannot be upgraded or treated as V3. The receipt remains append-only, and a first valid apply still changes exactly four rows: two supersession inserts, the one Berlin legacy-import CAS update with `opportunity_id` remaining `NULL`, and one immutable repair receipt insert. Exact verified replay makes zero writes. V3 adds no repair write to either row-excluded table. The CLI still defaults to a read-only, existing-file, query-only, consistent-transaction preview. On successful apply or verified replay, stdout exposes only `status`, `mode`, `applied`, `mutationCount`, `manifestId`, `planChecksum`, and `integrity.valid`, `integrity.quickCheck`, `integrity.foreignKeyViolationCount`; no internal rows or raw identities belong in that output.

The sanitized pre-V3 evidence showed the immutable backup at 137 `analytics_events` and 105 `contact_rate_limit_events` rows, versus live at 140 and 107. The complete schema digest and every fixed target raw-row digest matched between backup and live; only those two operational row digests differed. The previously authorized production preview **never ran**: inspection stopped before that boundary. The evidence supports correcting V2's all-row authority, not treating a failed or completed preview as an apply prerequisite.

The corrected Fly v126 checkpoint and its backup/snapshot are **historical-only for V3**. After exact V3 implementation review, merge, and deployment, a new owner-authorized post-deploy recovery checkpoint must bind a fresh application-consistent SQLite backup, Fly snapshot, deployed release, and tooling SHA. The owner must then separately authorize one read-only V3 preview, independently review its exact canonical artifact and zero-mutation evidence, and separately authorize any apply against that reviewed artifact and fresh checkpoint. Post-apply verification is another gate. This documentation commit grants no production authorization: it authorizes no deploy, backup, snapshot, production access, preview, apply, retry, restore, reconciliation, automation change, or send. No ready-to-run production apply command or restricted raw identity preimage is provided here.
