...
## 20. 2026-09-20 coordinated V2 source-shape and runtime-safety correction

This section records the owner-approved UG-P7-01J addendum without rewriting the earlier design history. The approved addendum SHA-256 is `32973350304fd1e1ef5c1f4f0a9caab36cd293d0536d360af6155c935ae7b29a`. The owner selected Option 2: exact Berlin identity evidence stays outside Git, ordinary CI uses explicitly synthetic values, and merge readiness requires a restricted local acceptance gate that runs the final code and real SQLite adapter against the retained exact evidence. Hosted CI alone is not acceptance.

The V1 implementation assumptions about Pooler aliases, `financialProvenance`, and synthetic safety metadata are superseded as follows:

- Pooler listing authority accepts its retained supported marketplace URL with no manufactured aliases. Primary URL, listing aliases, and identity aliases are inspected independently. Malformed, unsupported, or conflicting evidence is retained as a blocker even when another representation is valid.
- Financial authority is the exact `metadata.dealHunter.raw["Annual Profit"]` value plus the stored projection. No `financialProvenance` or stored period is required or created; an unsupplied period remains unknown.
- Berlin's missing listing evidence is never positive authority. Acceptance requires the fixed opportunity/survivor/duplicate/import IDs, active approved primary, exact compatible ownership, no loser primary/owner/listing/alias/source-record evidence, the fixed raw loser deal-key SHA-256 `3d9a1bfb64efd766a7bc3dd8c584a7fc0aab58a74cbcbd377893ed42bd65f733`, exact `sheet-0/csv/18` scalar pointers, exactly one matching survivor deal-key alias, exact legacy and canonical import relationships, and an unambiguous survivor marketplace identity of `costar:2436873`. Raw strings are type-checked and hashed as unmodified UTF-8 bytes.
- The canonical import's durable `opportunity_id` column carries the opportunity relationship. Its retained metadata has no `opportunityId`; its normalized `listing_identity`, listing URL, and deal key must agree with the survivor evidence. The legacy import likewise has no metadata opportunity owner and must remain `opportunity_id = NULL`.

The repair now binds one deterministic `crm-duplicate-consolidation-runtime-safety-v1` authority. It contains only four reviewed facts: SQLite `deal_hunter_cim_safety_settings/global/outreach_paused = true`, SQLite `deal_hunter_automation_settings/cim-initial-outreach/paused = true`, effective `dealHunter.cimFollowUp.enabled = false`, and effective `dealHunter.cimAutomation.schedulerEnabled = false`. Durable facts include exact raw-row digests. Config facts include the exact source paths, environment-variable identities, normalized booleans, and validated absent/empty/explicit-false representation. Secrets, unrelated config, process identity, and sample time are excluded, and no digest hashes itself.

Authority comparison is deliberately two-stage: current config-only authority is compared with reviewed config-only authority; then current complete four-source authority is compared with the reviewed complete authority. Preview captures all database facts in one read-only, file-existing, query-only transaction. Backup reconstruction reads durable rows from the backup but uses reviewed config facts only as external facts; it never claims the backup observed runtime configuration. Before writable construction, the CLI validates current config and a short-lived read-only durable-row inspection. The writable instance receives the same config/environment selection, the service compares its captured config authority, and `BEGIN IMMEDIATE` independently rebuilds complete authority before receipt lookup or any write. Replay uses the state-independent safety inspection and therefore never skips current safety.

The corrected artifact boundary is:
...
- checkpoint envelope: unchanged `crm-duplicate-consolidation-checkpoint-v1`.

V1 artifacts and confirmations, missing or unknown authority fields, malformed configuration, authority drift, and unsafe durable rows refuse before writable construction. The first valid transaction remains exactly two supersession inserts, one Berlin legacy-import CAS update, and one immutable receipt insert. No contact, history, source, canonical opportunity, alias, score, communication, CIM, provider, reconciliation, or unrelated row mutation is added. Runtime flags and one CLI process do not prove global process quiescence; the later operational preflight remains a separate owner gate.
