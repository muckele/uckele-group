...
- Tasks 1–8 may create intermediate development savepoints while the V3 contract is being assembled. No intermediate commit is a releasable V3 revision. No implementation PR, merge, deployment, or production use is permitted until all contract/storage/CLI tasks, regressions, public gates, restricted exact-head acceptance, and whole-branch review have passed.
...
| `docs/reviews/crm-duplicate-consolidation-implementation-verification.md` | Append the V3 public/focused verification record available before restricted acceptance, and state that exact-head restricted evidence remains a subsequent external gate; no raw evidence or later restricted hashes enter this Git record. |
...
**Interfaces:** Consumes only the implemented V3 code, the approved V3 spec, and public/focused test evidence available **before** restricted acceptance. Produces a current V3 addendum without rewriting historical V1/V2 provenance, claiming Task 11 results, or suggesting that this plan authorizes a production operation.
...
- [ ] **Step 2: GREEN documentation.** Append a `## V3 row-authority correction` section to the runbook and a dated V3 verification section to the review record. State the 137/105 backup versus 140/107 live evidence, matching full schema/target hashes, and that the authorized preview never ran. Explain schema-all/rows-minus-exact-two/reference-minus-exact-two, v3 versions, immutable receipt and four writes, historical-only v126 checkpoint, and the required fresh post-deploy checkpoint. Record only completed public/focused tests and include this factual sentence in the verification record: “Mandatory restricted exact-evidence acceptance remains a subsequent release gate and is not claimed by this repository record.” State that final exact-head restricted harness, report, package, and archive identities remain external to Git for the Task 11 owner handoff or later separately authorized implementation PR metadata. State that the documentation commit grants no production authorization. Preserve preview→owner review→separate apply and the CLI's safe output contract; never include a ready-to-run production apply command or restricted preimage.
- [ ] **Step 3: Verify/review.** Rerun the Step 1 documentation check and `git diff --check`; inspect `git diff -- docs/crm-duplicate-consolidation-repair.md docs/reviews/crm-duplicate-consolidation-implementation-verification.md` for privacy, historical wording, no unearned restricted-acceptance claim, and no accidental operational authorization. Ask a fresh reviewer to verify every claimed result existed before this documentation commit and the Task 11 gate is explicitly pending.
...
**Files:** No routine repository change after Task 10. Retain the final private harness, report, package, hashes, and archive outside Git. Report their exact identities in the owner handoff or later separately authorized implementation PR metadata, not by making a post-acceptance documentation commit. No production read or write.

**Interfaces:** Consumes the actual final implementation HEAD **including Task 10's documentation commit**, retained exact identity evidence, and a new private V3 derivative of the external privacy-reviewed harness. Produces a privacy-safe exact-head attestation for owner/independent review. Do not push or open/update a product-code PR until a later execution authorization explicitly permits it.

- [ ] **Step 1: Public gates at Node 22.** First record `git rev-parse HEAD`; it must be the final repository HEAD including Task 10's documentation commit. Run these exact commands from that isolated implementation worktree, stopping on any failure: `npm ci`; `npm audit --omit=dev`; `npm run lint`; `node --test test/crmDuplicateConsolidationV3Contract.test.js test/crmDuplicateConsolidationV2Contract.test.js test/crmDuplicateConsolidationRepair.test.js test/crmDuplicateConsolidationRepairSafety.test.js test/crmDuplicateConsolidationCli.test.js`; `npm test`; `npm run test:ui`; `npm run build`; `npm run test:browser`; `git diff --check`. Record command, exit status, test count, and the same exact HEAD. Dependency installation is **only for the later authorized execution gate**, not this plan-writing task.
- [ ] **Step 2: Prepare the restricted exact-head gate.** Resolve the retained external package through its private manifest and verify raw-evidence and harness SHA-256 before use. The retained `ug-p7-01n-restricted-acceptance.mjs` accepts `--repository`, `--evidence`, and `--report` but pins an old `TOOLING`/`TREE`/`BASE`/`STARTING_HEAD`; it **cannot** be run unchanged as V3 evidence. Make a separately named private V3 derivative (mode `0600`, outside Git), bind those identities to the Step 1 HEAD, preserve its real CLI/service/adapter assertions, and add the V3 cases, including the dedicated real-CLI preview bracket in Step 3. Hash the new derivative and record provenance. Do not copy raw evidence into Git or logs. Include three independent synthetic volatile-growth cases (analytics only, rate-limit only, both) using retained fixed Berlin identity only inside private disposable databases; require equal `authorityLogicalDigest`, `authorityTableDigests`, reference inventory, and `planChecksum`, while authoritative-row or schema drift refuses. Do not run the derivative until the Step 3 bracket is implemented and independently checked.
- [ ] **Step 3: Bracket exactly one real CLI preview before any first apply.** In the private V3 derivative's positive flow, immediately before its existing `spawnSync(nodePath, [cliPath, ...commonCliArgs(fixture, checkpoint)], ...)` preview call, capture a read-only fingerprint of the disposable exact-identity SQLite fixture. Capture the **same** fields immediately after that subprocess returns and before writing the reviewed artifact or invoking apply. The fingerprint contains `inspection.database.authorityLogicalDigest`, `inspection.schema.digest`, the raw digests of the four fixed contact rows and Berlin legacy import, counts of `crm_submission_supersessions` and `deal_hunter_cim_repair_manifests` rows with `mode = 'crm-duplicate-consolidation'`, `PRAGMA quick_check`, the number of `foreign_key_check` rows, and separate diagnostic counts for `analytics_events` and `contact_rate_limit_events`. Use the approved descriptor IDs, not a raw identity preimage. A concrete harness-local helper, using its existing `rawDatabase` and imported read-only adapter, is:

```js
async function cliPreviewFingerprint(fixture) {
  const reader = createSqliteCrmDuplicateConsolidationReadOnlyStorage(fixture.config, {
    environment: {},
  });
  let inspection;
  try { inspection = await reader.inspectCrmDuplicateConsolidation(); }
  finally { reader.close(); }
  const targetKeys = CRM_DUPLICATE_CONSOLIDATION_DESCRIPTOR.pairs.flatMap((pair) => [
    `contact_submissions:${pair.survivorSubmissionId}`,
    `contact_submissions:${pair.supersededSubmissionId}`,
  ]).concat(`deal_hunter_crm_imports:${CRM_DUPLICATE_CONSOLIDATION_DESCRIPTOR.berlinImport.id}`);
  const raw = new Map(inspection.rawRows.map((row) => [`${row.table}:${row.id}`, row.digest]));
  requireCondition(targetKeys.length === 5 && targetKeys.every((key) => raw.has(key)),
    'CLI_PREVIEW_TARGET_FINGERPRINT_INCOMPLETE');
  const counts = rawDatabase(fixture.sqlitePath, (db) => ({
    relations: db.prepare('SELECT COUNT(*) AS count FROM crm_submission_supersessions').get().count,
    receipts: db.prepare(`SELECT COUNT(*) AS count FROM deal_hunter_cim_repair_manifests
      WHERE mode = 'crm-duplicate-consolidation'`).get().count,
    ...
    foreignKeyViolationCount: db.pragma('foreign_key_check').length,
  }), { readonly: true });
  return {
    authorityLogicalDigest: inspection.database.authorityLogicalDigest,
    schemaDigest: inspection.schema.digest,
    targetRawDigests: Object.fromEntries(targetKeys.map((key) => [key, raw.get(key)])),
    relationCount: counts.relations,
    receiptCount: counts.receipts,
    quickCheck: counts.quickCheck,
    foreignKeyViolationCount: counts.foreignKeyViolationCount,
    volatileDiagnostic: { analytics: counts.analytics, rateLimit: counts.rateLimit },
  };
}
```

  Require before/after equality of the authoritative digest, schema digest, all five target/import raw hashes, relation count, and consolidation receipt count; require `quickCheck === 'ok'` and `foreignKeyViolationCount === 0` on both sides. Parse the real CLI stdout and require `mode === 'preview'`, `applied === false`, `blockers` exactly `[]`, and `connection.readonly`, `fileMustExist`, `queryOnly`, and `consistentReadTransaction` all `true`. Record the two volatile counts separately; **do not** compare them for equality and do not compare all-table digests. Any movement must be attributable to a harness-controlled volatile write plus the preview's read-only/query-only evidence; unaccounted movement is a stop, not proof of preview safety. The report and owner handoff must include this preview before/after equality proof separately from apply and replay accounting. Only after these assertions may the derivative start first apply. Assign `V3_HARNESS`, `RETAINED_EVIDENCE`, and `V3_REPORT` to manifest-verified absolute private paths, then run `node "$V3_HARNESS" --repository "$PWD" --evidence "$RETAINED_EVIDENCE" --report "$V3_REPORT"` against the exact Step 1 HEAD.
- [ ] **Step 4: Separately verify first apply and replay.** Through the **real CLI subprocess, service, read-only adapter, and `createSqliteStorage`**, verify immutable backup reproduction, first apply exactly four rows, no repair-origin volatile write, Berlin import still unowned (`opportunity_id = NULL`), zero-write replay after volatile growth, authoritative/schema/safety/identity drift refusal, five rollback hooks (writes 1–4 and final postcondition), immutable receipt UPDATE/DELETE refusal, clean `quick_check`/FK checks, clean supersession audit, and unchanged generic CRM audit. Keep first-apply accounting and replay zero-write accounting as two gates distinct from the Step 3 preview bracket. For each, record authoritative digest, target/import raw hashes, relation/receipt counts, and volatile counts separately; do not require an all-table digest to remain unchanged after deliberately adding volatile rows.
- [ ] **Step 5: Restricted privacy and provenance.** Scan actual CLI stdout/stderr, new Git blobs, and the private report for every retained sensitive value in plain, Base64, hex, and URI forms. Require zero unapproved matches and no raw preimage in tracked Git. Retain the private report/package/archive in durable owner-reviewable storage with file modes and independent archive readback/checksum; record only privacy-safe paths/hashes and counts in the owner handoff or later separately authorized implementation PR metadata. If Task 11 produces only final acceptance evidence, **no repository commit follows**. A failed access, identity, privacy, or acceptance gate is a stop, never permission to weaken a guard.
- [ ] **Step 6: Whole-branch review and handoff.** Request a fresh independent whole-branch review of the exact final head, including the five Review Focus failures, all 42 spec scenarios, separate preview/apply/replay proofs, real-adapter/restricted evidence, version boundary, four-write ledger, safety, receipt/replay, privacy, and no production effects. If a real product defect is found, make a normal focused correction commit; update repository documentation only if that correction changes the documented contract, then rerun **all** exact-head public/restricted gates and whole-branch review on the new HEAD. Do not make a routine documentation commit merely to insert final acceptance hashes. Verify `git diff --check`, exact branch diff, clean worktree, and approved design commit in history. Push/open or update an implementation PR **only if later owner authorization explicitly permits it**. Stop for owner review; do not merge/deploy/preview/apply production.
